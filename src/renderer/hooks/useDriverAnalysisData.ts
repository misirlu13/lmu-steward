...
export const useDriverAnalysisData = ({
  ...
  const currentSessionLogData = useMemo(() => {
    const sessionKey =
      ...
      CONSTANTS.SESSION_TYPE_MAPPINGS[
        currentReplay.metadata
          .session as keyof typeof CONSTANTS.SESSION_TYPE_MAPPINGS
      ];
  ...
  const selectedSessionDriver = useMemo(() => {
    ...
    const byName = sessionDrivers.find(
      (entry) =>
        normalizeDriverName(String(entry?.Name ?? '')) ===
  ...
  }, [currentSessionLogData, driver.driverName, driver.position]);

  const selectedDriverIdentity = useMemo(() => {
    return {
      normalizedName: normalizeDriverName(
        String(selectedSessionDriver?.Name ?? driver.driverName ?? ''),
      ),
      driverSid: String(
        selectedSessionDriver?.ID ?? driver.driverSid ?? '',
      ).trim(),
    };
  }, [driver.driverName, driver.driverSid, selectedSessionDriver]);

  const matchesSelectedIncidentDriver = useCallback(
    (incidentDriver: { displayName?: string }) => {
      if (!incidentDriver) {
        return false;
      }

      const normalizedName = normalizeDriverName(
        String(incidentDriver.displayName ?? ''),
      );
  ...
  const selectedDriverIsAi = useMemo(
    () =>
      String(
        selectedSessionDriver?.isPlayer ??
          selectedSessionDriver?.IsPlayer ??
          '',
      ).trim() === '0',
  ...
  const incidents = useMemo<ReplayIncidentEvent[]>(() => {
    if (!currentSessionLogData?.Stream || !selectedSessionDriver) {
      ...
      return fallback.filter((incident) =>
        incident.drivers.some((entry) => matchesSelectedIncidentDriver(entry)),
      );
    }
    ...
    const targetName = normalizeDriverName(
      String(selectedSessionDriver?.Name ?? driver.driverName ?? ''),
    );
    ...
    const buildTimelineDriver = (
      ...
      return {
        displayName:
          cleanedName.replace(/#\d+$/, '').trim() || 'Unknown Driver',
        carNumber: String(matchedDriver?.CarNumber ?? fallbackCarNumber ?? ''),
        carClass: String(
          matchedDriver?.CarClass ?? driver.carClass ?? 'Unknown',
        ),
        slotId:
          String(matchedDriver?.SlotID ?? matchedDriver?.slotID ?? '').trim() ||
          undefined,
        driverSid:
    ...
    const trackLimitEvents = toArray(currentSessionLogData.Stream.TrackLimits)
      .map((item, index: number) => {
        ...
        const driversForEvent = [
          buildTimelineDriver(rawName, undefined, item?.ID),
        ];
        return buildEvent(
          'track-limit',
          item,
    ...
    const incidentEvents = toArray(currentSessionLogData.Stream.Incident)
      .map((item, index: number) => {
        ...
        const forceSummary =
          buildDriverAnalysisIncidentForceSummary(sourceText);
        const baseDescription =
          extractDriverAnalysisIncidentDescription(sourceText);
        const description =
        ...
        const includesTarget =
          ...
          (secondary &&
            isTargetDriver(secondary.name, secondary.carNumber, null));
  ...
  const isSelectedIncidentDriver = useCallback(
    (incidentDriver: {
      displayName: string;
      carNumber: string;
      driverSid?: string;
    }) => matchesSelectedIncidentDriver(incidentDriver),
    [matchesSelectedIncidentDriver],
  );
  ...
  const faultIncidentSummary = useMemo(
    () => ({
      'track-limit': faultScopeEvents.filter(
        (entry) => entry.type === 'track-limit',
      ).length,
      collision: faultScopeEvents.filter((entry) => entry.type === 'collision')
        .length,
      penalty: faultScopeEvents.filter((entry) => entry.type === 'penalty')
        .length,
    }),
    [faultScopeEvents],
  );
  ...
  const faultTotalIncidents = useMemo(
    () =>
      faultIncidentSummary['track-limit'] +
      faultIncidentSummary.collision +
      faultIncidentSummary.penalty,
  ...
  const faultCollisionStats = useMemo(() => {
    const collisionEvents = faultScopeEvents.filter(
      (entry) => entry.type === 'collision',
    );

    const subjectCount = collisionEvents.filter((entry) =>
    ...
    const secondaryCount = collisionEvents.filter(
      (entry) =>
        ...
        entry.drivers
          .slice(1)
          .some((incidentDriver) => isSelectedIncidentDriver(incidentDriver)),
    ).length;
    ...
    const topCounterparty =
      [...counterpartyCounts.entries()].sort(
  ...
  }, [faultScopeEvents, isSelectedIncidentDriver]);
  ...
  const faultPenaltyReasons = useMemo(() => {
    ...
    streamPenalties.forEach((entry) => {
      const nameMatch =
        normalizeDriverName(String(entry?.Driver ?? '')) ===
        selectedDriverIdentity.normalizedName;
      const sidMatch =
        selectedDriverIdentity.driverSid &&
        String(entry?.ID ?? '').trim() === selectedDriverIdentity.driverSid;
      ...
      const reason =
        String(entry?.Reason ?? entry?.Penalty ?? 'Unspecified').trim() ||
        'Unspecified';
  ...
  const faultRiskIndex = useMemo(() => {
    const lapsCompleted = Number(
      selectedSessionDriver?.Laps ??
        toArray(selectedSessionDriver?.Lap).length ??
        0,
    );
  ...
  const topCounterpartyText = useMemo(() => {
    ...
    const involvementCount =
      faultCollisionStats.subjectCount + faultCollisionStats.secondaryCount;
  ...
  const lapBreakdownRows = useMemo<DriverLapBreakdownRow[]>(() => {
    ...
    const lapEntries = toArray(selectedSessionDriver?.Lap)
      .map((lap, index: number) => {
        ...
        const lapTime =
          Number.isFinite(lapTimeSeconds) && lapTimeSeconds > 0
            ? lapTimeSeconds
            : null;
      ...
      .sort(
        (left: { lapNumber: number }, right: { lapNumber: number }) =>
    ...
    toArray(stream?.TrackLimits).forEach((entry) => {
      ...
      const resolvedLap =
        Number.isFinite(lap) && lap > 0
          ? lap
          : getLapFromDriverEt(selectedSessionDriver, Number(entry?.et));
      if (resolvedLap) {
  ...
  }, [currentSessionLogData, driver.driverName, selectedSessionDriver]);
