...
export const resolveIncidentFocusTarget = (
  ...
  const selectedName = normalizeDriverName(
    String(selectedDriver.driverName ?? ''),
  );
  ...
  const fallbackDriver = incidentDrivers.find((entry) => {
    return Boolean(
      normalizeIdentityValue(entry.slotId) ||
        normalizeIdentityValue(entry.driverSid),
    );
  });
...
export const formatLapTime = (
  lapTimeSeconds: number | null | undefined,
): string => {
  if (
    !Number.isFinite(lapTimeSeconds) ||
    !lapTimeSeconds ||
    lapTimeSeconds <= 0
  ) {
    return '--:--.---';
...
export const getLapFromDriverEt = (
  ...
  const laps = toArray(driver?.Lap)
    ...
    .filter(
      (entry) => Number.isFinite(entry.lapNum) && Number.isFinite(entry.et),
    )
...
export const extractIncidentImpactForce = (
  value: string,
): number | undefined => {
