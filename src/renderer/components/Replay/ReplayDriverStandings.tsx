...
import {
  CarClassBadge,
  getCarClassBadgeColor,
} from '../CarClassBadge/CarClassBadge';
...
const classOrder = ['HYPERCAR', 'HY', 'LMP2', 'P2', 'LMP3', 'GT3', 'GTE'];
...
export const ReplayDriverStandings: React.FC<ReplayDriverStandingsProps> = ({
  ...
  const carClasses = useMemo(
    () =>
      [
        ...new Set(standings.map((entry) => entry.carClass).filter(Boolean)),
      ].sort(
  ...
  useEffect(() => {
    if (
      (!hasLimitedDataRecords || !canShowLimitedDataFilter) &&
      hideLimitedData
    ) {
  ...
  const visibleStandings = useMemo(() => {
    ...
    return [...byClass].sort((left, right) => {
      if (selectedClass === 'all') {
        const classDiff =
          getClassSortIndex(left.carClass) - getClassSortIndex(right.carClass);
        if (classDiff !== 0) {
          return classDiff;
        }
      }
                ...
                >
                  ...
                  <TableCell>
                    <Typography
                      ...
                      fontWeight={700}
                      color={
                        entry.incidents > 0 ? 'warning.main' : 'text.primary'
                      }
                    >
                      {entry.incidents}
                    </Typography>
