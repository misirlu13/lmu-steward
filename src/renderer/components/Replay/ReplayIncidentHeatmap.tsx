...
const clampZoom = (value: number) =>
  Number(Math.max(MIN_ZOOM, Math.min(MAX_ZOOM, value)).toFixed(1));
...
export const ReplayIncidentHeatmap: React.FC<ReplayIncidentHeatmapProps> = ({
  ...
  const [zoomLevel, setZoomLevel] = useState<number>(
    Math.max(MIN_ZOOM, Number.parseFloat(zoomLabel) || 1),
  );
  ...
  const applyWheelZoomDelta = useCallback((deltaY: number) => {
    setZoomLevel((previousZoomLevel) =>
      clampZoom(
        previousZoomLevel + (deltaY < 0 ? WHEEL_ZOOM_STEP : -WHEEL_ZOOM_STEP),
      ),
    );
  }, []);

  const handleFullscreenWheelZoom = (
  ...
  useEffect(() => {
    ...
    return () => {
      interactionLayer.removeEventListener(
        'wheel',
        handleNativeWheelZoom,
        true,
      );
    };
  }, [isExpanded, applyWheelZoomDelta]);
      ...
      >
        ...
        >
          ...
          <Stack
            ...
            marginLeft="7px"
          >
            {(['minor', 'serious', 'critical'] as HeatmapSeverity[]).map(
              (severity) => (
                <Stack
                  key={severity}
                  direction="row"
