...
export const DriverAnalysisView: React.FC = () => {
	...
	const { replayHash, driverId } = useParams<{ replayHash: string; driverId: string }>();
	...
	return (
		<Box sx={{ paddingBottom: { xs: '260px', md: '180px' } }}>
			<ViewHeader
				...
				title={
					<Stack direction="row" spacing={1} alignItems="center" useFlexGap>
						<Typography variant="h5">{driver.driverName} • Driver Analysis</Typography>
			...
			{isQuickViewModeActive ? (
				<Box sx={{ mt: -1, mb: 2, px: 0.5 }}>
					<Typography variant="body2" color="text.secondary">
						Quick View is enabled. Replay playback actions are unavailable until replay loading is started from Session Analysis.
					</Typography>
				</Box>
			) : null}
			...
			{!isQuickViewModeActive ? (
				...
				<Paper variant="outlined" sx={{ mt: 2, borderColor: 'divider', borderRadius: 2, p: 2 }}>
					<Typography variant="body2" color="text.secondary">
						Replay jump controls are unavailable in Quick View mode. Load the replay from
						Session Analysis to enable playback-linked driver analysis controls.
					</Typography>
				</Paper>
			)}
		</Box>
